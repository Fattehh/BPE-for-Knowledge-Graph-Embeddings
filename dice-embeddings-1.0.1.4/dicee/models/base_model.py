...
class BaseKGELightning(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx=None):
        ...
        if self.args["multiple_bpe_loss"]:
            loss_batch += self.mse_loss
            self.mse_loss = 0
...
class BaseKGE(BaseKGELightning):
    def __init__(self, args: dict):
        ...
        if self.byte_pair_encoding and self.args['model'] != "BytE":
            ...
            match self.args.get("bpe_with_RNN"):
                case "" | "Linear":
                    self.lf = nn.Sequential(nn.Linear(self.embedding_dim * self.max_length_subword_tokens,
                                                      self.embedding_dim, bias=False))
                # RNN implemented by Fatos
                case "RNN":
                    self.rnn_to_emb = nn.Sequential(nn.RNN(self.embedding_dim, self.embedding_dim, batch_first=True))
                case "GRU":
                    self.rnn_to_emb = nn.Sequential(nn.GRU(self.embedding_dim, self.embedding_dim, batch_first=True))
                case "LSTM":
                    self.rnn_to_emb = nn.Sequential(nn.LSTM(self.embedding_dim, self.embedding_dim, batch_first=True))
            if self.args["scoring_technique"] in ["AllvsAll", "KvsAll"]:
                ...
                self.bpe_entity_to_idx = {shaped_bpe_ent: idx for idx, (str_ent, bpe_ent, shaped_bpe_ent) in
                                          enumerate(self.args["ordered_bpe_entities"])}
                # Creating ordered_BPE from ent->idx drops duplicates caused by truncation
                self.ordered_bpe_entities = torch.tensor([elem[2] for elem in self.args['ordered_bpe_entities']],
                                                         dtype=torch.long)
                if self.args.get("bpe_with_RNN") in ["RNN", "GRU", "LSTM"]:
                    self.ordered_ent_lengths = [min(len(elem[1]), len(elem[2])) for elem in
                                                self.args['ordered_bpe_entities']]
                self.ordered_bpe_relations = torch.tensor([elem[2] for elem in self.args['ordered_bpe_relations']],
                                                          dtype=torch.long)
                # self.ordered_bpe_entities = torch.tensor(list(self.bpe_entity_to_idx.keys()), dtype=torch.long)
            if self.args["multiple_bpe_loss"]:
                self.duplicate_tokenizations = torch.tensor(self.args['duplicate_tokenizations'], dtype=torch.long)
                self.dupl_num = self.duplicate_tokenizations.shape[0]
                self.mse_from_means = torch.nn.MSELoss()
                self.last_loss = 0
                self.mse_loss = 0
    ...
    def forward_byte_pair_encoded_k_vs_all(self, x: torch.LongTensor):
        ...
        if self.args.get("bpe_with_RNN") in ["", "Linear"]:
            # (4) Reshaping (3) into row vectors (E, T*D) .
            E = E.reshape(len(E), T * D)  # N, L, Emb
            ...
            bpe_head_lengths = [self.args['length_dict'][tuple(head.tolist())] for head in x[:, 0]]
            bpe_rel_lengths = [self.args['length_dict'][tuple(head.tolist())] for head in x[:, 1]]
            if self.args.get("bpe_with_RNN") in ["RNN", "GRU"]:
                bpe_head_ent_emb = self.rnn_to_emb(
                    pack_padded_sequence(bpe_head_ent_emb, lengths=bpe_head_lengths, batch_first=True,
                                         enforce_sorted=False))[1][0]
                bpe_rel_ent_emb = self.rnn_to_emb(
                    pack_padded_sequence(bpe_rel_ent_emb, lengths=bpe_rel_lengths, batch_first=True,
                                         enforce_sorted=False))[1][0]
                E = self.rnn_to_emb(
                    pack_padded_sequence(E, lengths=self.ordered_ent_lengths, batch_first=True, enforce_sorted=False))[
                    1][0]

            else:
                bpe_head_ent_emb = self.rnn_to_emb(
                    pack_padded_sequence(bpe_head_ent_emb, lengths=bpe_head_lengths, batch_first=True,
                                         enforce_sorted=False))[1][0][0]
                bpe_rel_ent_emb = self.rnn_to_emb(
                    pack_padded_sequence(bpe_rel_ent_emb, lengths=bpe_rel_lengths, batch_first=True,
                                         enforce_sorted=False))[1][0][0]
                E = self.rnn_to_emb(
        ...
        if self.args["multiple_bpe_loss"]:
            if self.training and self.last_loss > len(self.trainer.train_dataloader.batch_sampler):
                self.mse_loss = 0
                # We had to add duplicated entities/relations, so now we subtract them.
                self.duplicate_tokenizations = self.duplicate_tokenizations.to(self.device)
                multi_Tok = self.token_embeddings(self.duplicate_tokenizations)
                if self.args.get("bpe_with_RNN") in ["", "Linear"]:
                    T, D = multi_Tok.shape[-2:]
                    multi_Tok = multi_Tok.reshape(list(multi_Tok.shape[0:2]) + [T * D])
                    multi_Tok = multi_Tok.flatten(end_dim=1)
                    multi_Tok = self.lf(multi_Tok)
                    multi_Tok = multi_Tok.unflatten(0, (self.dupl_num, multi_Tok.shape[0] // self.dupl_num))
                elif self.args.get("bpe_with_RNN") in ["RNN", "GRU"]:
                    multi_Tok = multi_Tok.flatten(end_dim=1)
                    multi_Tok = self.rnn_to_emb(
                        pack_padded_sequence(multi_Tok, lengths=self.args["duplicate_lengths"], batch_first=True,
                                             enforce_sorted=False))[1][0]
                    multi_Tok = multi_Tok.unflatten(0, (self.dupl_num, multi_Tok.shape[0] // self.dupl_num))
                elif self.args.get("bpe_with_RNN") in ["LSTM"]:
                    multi_Tok = multi_Tok.flatten(end_dim=1)
                    multi_Tok = self.rnn_to_emb(
                        pack_padded_sequence(multi_Tok, lengths=self.args["duplicate_lengths"], batch_first=True,
                                             enforce_sorted=False))[1][0][0]
                ...
                multi_mean = (multi_Tok.mean(dim=0) -
                              multi_Tok[0].mul(
                                  torch.tensor(self.args["duplicate_additions"]).to(self.device).repeat((self.embedding_dim, 1)).T))
                self.mse_loss = self.mse_from_means(multi_Tok, multi_mean.repeat(self.duplicate_tokenizations.shape[0], 1, 1))
                self.last_loss = 0
            self.last_loss += 1
    ...
    def validation_step(self, batch, batch_idx):
        # (1) Enable eval mode.
        self.eval()
        loss = self.trainer.evaluator.eval(dataset=self.trainer.dataset, trained_model=self,
                                           form_of_labelling=self.trainer.form_of_labelling,
                                           during_training=True)["Val"]["MRR"]
        # (4) Enable train mode.
        self.train()
        self.log("val_MRR", loss)
