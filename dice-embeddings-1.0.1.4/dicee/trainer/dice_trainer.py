...
def initialize_trainer(args, callbacks):
    if args.trainer == 'torchCPUTrainer':
        ...
        if kwargs["min_epochs"] < kwargs["max_epochs"]:
            limit_val_batches = 1.0
            callbacks.append(EarlyStopping(monitor="val_MRR", mode="max", patience=10))
        else:
            limit_val_batches = 0
        ...
        return pl.Trainer(accelerator=kwargs.get("accelerator", "auto"),
                          ...
                          barebones=False, enable_checkpointing=False)
    else:
        ...
        if self.args.num_folds_for_cv == 0:
            ...
            self.trainer.fit(model, train_dataloaders=self.initialize_dataloader(
                self.initialize_dataset(knowledge_graph, form_of_labelling)),
                             val_dataloaders=torch.utils.data.DataLoader([1],batch_size=1))  # knowledge_graph.raw_valid_set)
            if torch.cuda.is_available():
                model.to("cuda:0")
            return model, form_of_labelling
        else:
