...
def preprocesses_input_args(args):
    ...
    assert args.min_epochs is None or isinstance(args.min_epochs, int)
    if args.min_epochs is None or args.min_epochs > args.max_epochs:
    ...
    assert args.multiple_bpe_encodings > 0
    if type(args.multiple_bpe_loss) is not bool:
        args.multiple_bpe_loss = args.multiple_bpe_loss == 'True'
    return args
